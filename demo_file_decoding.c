...
{
    char ofn[128] = {0};
    sprintf(ofn, "%s_%d.yuv", ctx->out_filename, _frame_idx);
    ...
    if(fp != NULL) {
        ...
        fclose(fp);
        char msg[256] = {0};
        sprintf(msg, "{\"w\": %d, \"h\": %d, \"format\": %d, \"data\": [\"%s_%d.yuv\"]}", 
                        ctx->width, ctx->height, 1, ctx->out_filename, _frame_idx);
        zmq_send(ctx->zmq_sck, msg, strlen(msg), 0);
        char buffer[8];
    ...
    while(true) {
        {
            ...
            sprintf(decctx->src_filename, "%s/%s", dctx->input, msg);
            sprintf(decctx->out_filename, "%s/%s", dctx->output, msg);
        }
...
void replier_process(demo_ctx *dctx) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        while (true) {
            char request[1024] = {0};//            int ret = replier.recv(&request, ZMQ_DONTWAIT);
            int ret = zmq_recv(dctx->zmq_sck, request, 1024, 0);
            if(ret > 0){
                break;
            }

            while (dctx->active < dctx->maxpross) {
                pid_t pid = fork();
                if (pid == -1) {
        ...
        char reply[1024] = {0};
        int  repsize = sprintf(reply, "%s", entry->d_name);
        zmq_send(dctx->zmq_sck, reply, repsize, 0);
        ++file_count;
        if ((file_count % 100) == 0) {
           sprintf_time(szTime, 32);
           printf("%s decode total files:%d\n", szTime, file_count);
        }
    }
    closedir(dir);
    sprintf_time(szTime, 32);
    printf("%s decode files end, total files:%d\n", szTime, file_count);
}
...
// ./demo_file_decoding /home/ams/data/ /tmp/data/ 4 1 3000 0

void usage(char* cmd) {
    printf("Usage: %s [-h] -i input -o output -p maxprocess -s skip -t maxtime -k -d\n", cmd);
    printf("Options:\n");
    printf("  -h              Print this help message.\n");
    printf("  -i input        input dir or stream address.\n");
    ...
    printf("\n");
    printf("Examples:\n");
    printf("  shell>  %s -i /root/video_99977 -o /mnt/ramfs/ -p 8 -s 30 -t 300 -d\n", cmd); 
    printf("  shell>  %s -i /root/video_99977 -o /mnt/ramfs/ -p 8 -k -t 300\n", cmd); 
}
...
int parse_opt(int argc, char* argv[], demo_ctx *dctx) {
    ...
    printf("%d %d %d %d\n", dctx->keyframe, dctx->maxpross, dctx->skip, dctx->maxtime);
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < dctx->maxpross; ++i) {
        pid_t pid = fork();
        if (pid == -1) {
            printf("Failed to fork.");
            return 1;
        } else if (pid == 0) {
            requester_process(dctx); // 子进程是 Requester
            exit(EXIT_SUCCESS);
        } else {
            ++dctx->active;
            ++dctx->index;
    ...
    int timeout = 0;
    zmq_setsockopt(dctx->zmq_sck, ZMQ_RCVTIMEO, &timeout, sizeof(timeout));
    std::thread t(check_thread, dctx);
